...
export default function EmployeeMedicineAdd() {
  ...
  const [branchId, setBranchId] = useState(null); // employee branch for direct stock
  ...
  // Fetch my profile to determine branchId (for direct branch stock addition)
  useEffect(() => {
    (async () => {
      try {
  ...
  useEffect(() => {
    ...
    const isPackContext =
      form.unit === "Packet" || form.unit === "Box" || packSize > 1;
    if (isPackContext) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (branchId) payload.branchId = branchId; // hint backend to treat as branch stock
      // Multiplier interpretation: if user typed a small number (<=3) as selling price treat it as factor * purchasePrice
      const pNum = parseFloat(payload.purchasePrice);
      if (Number.isFinite(pNum) && pNum > 0) {
        // Legacy single sellingPrice (if present and small)
        const legacyNum = parseFloat(payload.sellingPrice);
        if (legacyNum && legacyNum > 0 && legacyNum <= 3) {
          const adj = Math.round(pNum * legacyNum * 100) / 100;
          payload.sellingPrice = adj;
          if (!payload.sellingPriceBase) payload.sellingPriceBase = adj;
        }
        // Pack context price multiplier
