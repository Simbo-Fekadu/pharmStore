...
async function run() {
  const uri = process.env.MONGO || process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/pharmstore';
  ...
  const ops = [];
  ops.push(User.updateMany({ role: { $ne: 'super_admin' }, $or:[{ pharmacy: { $exists: false } }, { pharmacy: null }] }, { $set: { pharmacy: pid } }));
  ops.push(Branch.updateMany({ $or:[{ pharmacy: { $exists: false } }, { pharmacy: null }] }, { $set: { pharmacy: pid } }));
  ops.push(Medicine.updateMany({ $or:[{ pharmacy: { $exists: false } }, { pharmacy: null }] }, { $set: { pharmacy: pid } }));
  ops.push(Supplier.updateMany({ $or:[{ pharmacy: { $exists: false } }, { pharmacy: null }] }, { $set: { pharmacy: pid } }));
  ops.push(Request.updateMany({ $or:[{ pharmacy: { $exists: false } }, { pharmacy: null }] }, { $set: { pharmacy: pid } }));
  const results = await Promise.all(ops);
  const labels = ['users','branches','medicines','suppliers','requests'];
  labels.forEach((l,i)=> console.log(`[migrate] ${l} modified:`, results[i].modifiedCount));

  // Chat messages: assign pharmacy if missing
  const chatNoPharmacy = await ChatMessage.find({ $or:[{ pharmacy: { $exists:false } }, { pharmacy: null }] }).limit(2000).lean();
  if (chatNoPharmacy.length) {
    const senderIds = [...new Set(chatNoPharmacy.map(c=>c.senderId).filter(Boolean))];
    const senders = await User.find({ _id: { $in: senderIds } }).select('_id pharmacy').lean();
    const senderMap = senders.reduce((a,u)=>{ a[u._id.toString()] = u.pharmacy || pid; return a; }, {});
    ...
    console.log('[migrate] chat messages updated:', bulkRes.nModified || bulkRes.modifiedCount || 0);
  } else {
