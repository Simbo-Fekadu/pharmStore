...
import { StockLedger } from "../models/inventory.model.js";
...
async function buildDataset(type, scope) {
  ...
  if (type === "medicines") {
    const q = scope?.pharmacy ? { pharmacy: scope.pharmacy, isDeleted: false } : { isDeleted: false };
...
export const exportData = async (req, res) => {
  try {
    ...
    const allowedTypes = ["users", "medicines", "transactions"];
    ...
    if (fmt === "pdf") {
      ...
      doc.pipe(res);
      doc.fontSize(16).text(`${type.toUpperCase()} EXPORT`, { underline: true });
      doc.moveDown();
      ...
      columns.forEach((c, idx) => {
        doc.text(c, { continued: idx !== columns.length - 1, width: scaled[idx] });
      });
      doc.moveDown(0.5);
      rows.forEach((r) => {
        r.forEach((cell, idx) => {
          const val = cell == null ? "" : String(cell);
          doc.text(val, { continued: idx !== r.length - 1, width: scaled[idx] });
        });
        doc.moveDown(0.2);
      });
      doc.end();
